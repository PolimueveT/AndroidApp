...
public class Registro extends Activity implements OnClickListener {
	...
	/**
	 * Comprueba si todos los campos tienen informaci贸n "correcta" para enviar
	 * al servidor
	 * 
	...
	public boolean comprobar_datos() {
		...
		if (ok) {
			...
			// si las contrase帽as coinciden
			else if (!comprobar_pass()) {
				// toast contrase帽as no coinciden
				Toast notification = Toast.makeText(this,
						"La contrase帽a y la confirmaci贸n deben ser iguales",
						Toast.LENGTH_SHORT);
				notification.setGravity(Gravity.CENTER, 0, 0);
				notification.show();
				ok=false;
	...
	/**
	 * Recoge la informaci髇 que ha introducido el usuario en todos los campos y
	...
	/**
	 * Petici髇 post al servidor a la url urls[0] que recibe como par谩metro al
